...
学习深度学习项目：debug源码、读 pycharm debug 文档和评论，通读三遍，做笔记、其实就是 Evaluate Expression，这个要强大的多，可以运行公式，读文档； print 太低级了(可以结合 debug 和 print 看某一个具体变量，比如 print(net) 看网络结构)、添加 watch 看变量变化，可以添加 net、pysnooper 看某一个具体重要函数功能、读论文、可以用 step into 看更深层次的代码、pycharm 按 Ctrl 跳转源码、跳转以后可以使用 pysnooper、debug 有问题去 GitHub 的 issue 里去搜  
