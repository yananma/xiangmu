...
学习深度学习项目：debug源码、其实就是 Evaluate Expression print 还要重新运行才行(可以结合 debug 和 print 看某一个具体变量，比如 print(net) 看网络结构)、添加 watch 看变量变化、pysnooper 看某一个具体重要函数功能、读论文、可以用 step into 看更深层次的代码、pycharm 按 Ctrl 跳转源码、跳转以后可以使用 pysnooper、debug 有问题去 GitHub 的 issue 里去搜  
