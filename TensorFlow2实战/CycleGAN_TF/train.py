...
# PyTorch版推荐开源项目：https://github.com/junyanz/pytorch-CycleGAN-and-pix2pix
# 数据集下载：https://people.eecs.berkeley.edu/~taesung_park/CycleGAN/datasets/
...
flags.DEFINE_integer('epoch', 100,
                        'weight for backward c ycle loss (Y->X->Y), default: 10')
flags.DEFINE_float('learning_rate', 2e-4,
                      'initial lea  rning rate for Adam, default: 0.0002')
...
def train_step(G,F,D_Y,D_X,source,target,generator_g_optimizer,generator_f_optimizer,discriminator_x_optimizer,discriminator_y_optimizer,train_loss,lambda1,lambda2):
    with tf.GradientTape(persistent=True) as tape:
        ...
        gen_g_loss = tf.reduce_mean(tf.math.squared_difference(disc_fake_y, 1.0))
        gen_f_loss = tf.reduce_mean(tf.math.squared_difference(disc_fake_x, 1.0))
        #cycle loss		
        ...
        #diss loss
        disc_x_loss = 0.5*tf.reduce_mean(tf.math.squared_difference(disc_real_x,1.0)+tf.math.square(disc_fake_x))
        disc_y_loss = 0.5*tf.reduce_mean(tf.math.squared_difference(disc_real_y,1.0)+tf.math.square(disc_fake_y))		
        #reg loss
